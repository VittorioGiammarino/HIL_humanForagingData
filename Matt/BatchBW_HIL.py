...
class BatchHIL_param_simplified:
    ...
    def Pi_hi_bar(b, ot, ot_past, Pi_hi_parameterization, state, zeta, option_space):
        if b == True:
            o_prob_tilde = BatchHIL_param_simplified.Pi_hi(ot, Pi_hi_parameterization, state)
        elif ot == ot_past:
            o_prob_tilde = 1-zeta+np.divide(zeta,option_space)
        else:
            o_prob_tilde = np.divide(zeta,option_space)
    ...
    def Pi_b(b, ot_past, Pi_b_parameterization, state):
        Pi_b = Pi_b_parameterization(state)
        if ot_past==0:
            if b == True:
                b_prob = Pi_b[0,1]
    ...
    def Pi_combined(ot, ot_past, a, b, Pi_hi_parameterization, Pi_lo_parameterization, Pi_b_parameterization, state, zeta, option_space):
        Pi_hi_eval = np.clip(BatchHIL_param_simplified.Pi_hi_bar(b, ot, ot_past, Pi_hi_parameterization, state, zeta, option_space),0.0001,1)
        Pi_lo_eval = np.clip(BatchHIL_param_simplified.Pi_lo(a, Pi_lo_parameterization, state),0.0001,1)
        Pi_b_eval = np.clip(BatchHIL_param_simplified.Pi_b(b, ot_past, Pi_b_parameterization, state),0.0001,1)
        output = Pi_hi_eval*Pi_lo_eval*Pi_b_eval
    ...
    def ForwardRecursion(alpha_past, a, Pi_hi_parameterization, Pi_lo_parameterization,
        ...
        for i1 in range(option_space):
            ...
            for i2 in range(termination_space):
                ...
                Pi_comb = np.zeros(option_space)
                for ot_past in range(option_space):
                    Pi_comb[ot_past] = BatchHIL_param_simplified.Pi_combined(ot, ot_past, a, bt, 
                                                                             Pi_hi_parameterization, Pi_lo_parameterization[ot], Pi_b_parameterization, 
                                                                             state, zeta, option_space)
                alpha[ot,i2] = np.dot(alpha_past[:,0],Pi_comb)+np.dot(alpha_past[:,1],Pi_comb)  
        alpha = np.divide(alpha,np.sum(alpha))
                ...
                for i1_next in range(option_space):
                    ...
                    for i2_next in range(termination_space):
                        if i2_next == 1:
                            ...
                            b_next=False
                        beta[i1,i2] = beta[i1,i2] + beta_next[ot_next,i2_next]*BatchHIL_param_simplified.Pi_combined(ot_next, ot, a, b_next, 
                                                                                                                     Pi_hi_parameterization, Pi_lo_parameterization[ot_next], 
    ...
    def Alpha(self):
        ...
        for t in range(len(self.TrainingSet)):
            ...
            if t ==0:
                state = self.TrainingSet[t,:].reshape(1,len(self.TrainingSet[t,:]))
                action = self.Labels[t]
                alpha[:,:,t] = BatchHIL_param_simplified.ForwardFirstRecursion(self.mu, action, self.NN_options, 
                                                                               self.NN_actions, self.NN_termination, 
                                                                               state, self.zeta, self.option_space, self.termination_space)
            else:
                state = self.TrainingSet[t,:].reshape(1,len(self.TrainingSet[t,:]))
                action = self.Labels[t]
                alpha[:,:,t] = BatchHIL_param_simplified.ForwardRecursion(alpha[:,:,t-1], action, self.NN_options, 
                                                                          self.NN_actions, self.NN_termination, 
                                                                          state, self.zeta, self.option_space, self.termination_space)
    ...
    def Beta(self):
        ...
        for t_raw in range(len(self.TrainingSet)):
            ...
            state = self.TrainingSet[t,:].reshape(1,len(self.TrainingSet[t,:]))
            action = self.Labels[t]
            beta[:,:,t] = BatchHIL_param_simplified.BackwardRecursion(beta[:,:,t+1], action, self.NN_options, 
                                                                      self.NN_actions, self.NN_termination, state, self.zeta, 
                                                                      self.option_space, self.termination_space)
    ...
    def DoubleSmoothing(beta, alpha, a, Pi_hi_parameterization, Pi_lo_parameterization, 
        ...
        for i1_past in range(option_space):
            ...
            for i2 in range(termination_space):
                ...
                for i1 in range(option_space):
                    ot = i1
                    gamma_tilde[ot_past,i2] = gamma_tilde[ot_past,i2] + beta[ot,i2]*BatchHIL_param_simplified.Pi_combined(ot, ot_past, a, b, 
                                                                                                                          Pi_hi_parameterization, Pi_lo_parameterization[ot], 
                                                                                                                          Pi_b_parameterization, state, zeta, option_space)
                gamma_tilde[ot_past,i2] = gamma_tilde[ot_past,i2]*np.sum(alpha[ot_past,:])
        gamma_tilde = np.divide(gamma_tilde,np.sum(gamma_tilde))
    ...
    def Gamma(self, alpha, beta):
        ...
        for t in range(len(self.TrainingSet)):
            print('gamma iter', t+1, '/', len(self.TrainingSet))
            gamma[:,:,t]=BatchHIL_param_simplified.Smoothing(self.option_space, self.termination_space, alpha[:,:,t], beta[:,:,t])
    ...
    def GammaTilde(self, alpha, beta):
        ...
        for t in range(1,len(self.TrainingSet)):
            ...
            state = self.TrainingSet[t,:].reshape(1,len(self.TrainingSet[t,:]))
            action = self.Labels[t]
            gamma_tilde[:,:,t]=BatchHIL_param_simplified.DoubleSmoothing(beta[:,:,t], alpha[:,:,t-1], action, 
                                                                         self.NN_options, self.NN_actions, self.NN_termination, 
                                                                         state, self.zeta, self.option_space, self.termination_space)
        return gamma_tilde
...
# =============================================================================
        if version == 'classic':
            ...
            for i in range(option_space):
                if i == 0:
                    pi_b = NN_termination(TrainingSet[:],training=True)
                    loss = loss -kb.sum(gamma_tilde_reshaped[:,:,i]*kb.log(kb.clip(pi_b[:],1e-10,1.0)))/(T)
                if i == 1:
                    pi_b = 1-NN_termination(TrainingSet[:],training=True)
                    loss = loss -kb.sum(gamma_tilde_reshaped[:,:,i]*kb.log(kb.clip(pi_b[:],1e-10,1.0)))/(T)
                pi_lo = NN_actions[i](TrainingSet,training=True)
                loss = loss -(kb.sum(gamma_actions[:,:,i]*kb.log(kb.clip(pi_lo,1e-10,1.0))))/(T)
            ...
            loss_options = -kb.sum(gamma_reshaped_options*kb.log(kb.clip(pi_hi,1e-10,1.0)))/(T)
            loss = loss + loss_options
            Lb = BatchHIL_param_simplified.Regularizer_Lb(pi_hi)
            Lv = BatchHIL_param_simplified.Regularizer_Lv(pi_hi)
            DKL = BatchHIL_param_simplified.Regularizer_KL_divergence(NN_actions, TrainingSet, gamma_actions, auxiliary_vector)
            loss = loss + Lambda_Lb*Lb - Lambda_Lv*Lv - Lambda_DKL*DKL
            ...
            loss = loss + loss_options  
            # DKL = BatchHIL_param_simplified.Regularizer_KL_divergence(NN_actions, TrainingSet, gamma_actions, auxiliary_vector)
            # DKL_pi_hi = BatchHIL_param_simplified.Supervised_pi_hi(pi_hi, predictions)
            loss = loss #+ Lambda_DKL_pi_hi*DKL_pi_hi - Lambda_DKL*DKL
...
# =============================================================================
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0,self.option_space):
                self.optimizer.apply_gradients(zip(grads[i][:], self.NN_actions[i].trainable_weights))
            self.optimizer.apply_gradients(zip(grads[-1][:], self.NN_termination.trainable_weights))
            self.optimizer.apply_gradients(zip(grads[-2][:], self.NN_options.trainable_weights))
            print('options loss:', float(loss))
        ...
        for epoch in range(self.epochs):
            ...
            for n in range(n_batches):
                ...
                with tf.GradientTape() as tape:
                    ...
                    weights.append(self.NN_termination.trainable_weights)
                    tape.watch(weights)
                    loss = BatchHIL_param_simplified.Loss(gamma_tilde_reshaped[n*self.size_batch:(n+1)*self.size_batch,:,:], 
                                         gamma_reshaped_options[n*self.size_batch:(n+1)*self.size_batch,:], 
                                         gamma_actions[n*self.size_batch:(n+1)*self.size_batch,:,:], 
                                         auxiliary_vector[n*self.size_batch:(n+1)*self.size_batch,:],
                                         self.NN_termination, self.NN_options, self.NN_actions, self.size_batch, 
    ...
    def likelihood_approximation(self):
        ...
        for t in range(T):
            ...
            for o_past in range(self.option_space):
                for b in range(self.termination_space):
                    for o in range(self.option_space):
                        partial = partial + BatchHIL_param_simplified.Pi_hi(o_past, self.NN_options, state)*BatchHIL_param_simplified.Pi_combined(o, o_past, action, b, self.NN_options, self.NN_actions[o], 
                                                                                                                                                  self.NN_termination, state, self.zeta, self.option_space)
            if t == 0:
                likelihood = partial
            else:
                likelihood = likelihood+partial
...
# =============================================================================
        ...
        T = self.TrainingSet.shape[0]
        likelihood = [] #BatchHIL_param_simplified.likelihood_approximation(self)
            
        for n in range(N):
            print('iter Loss', n+1, '/', N)
        
            alpha = BatchHIL_param_simplified.Alpha(self)
            beta = BatchHIL_param_simplified.Beta(self)
            gamma = BatchHIL_param_simplified.Gamma(self, alpha, beta)
            gamma_tilde = BatchHIL_param_simplified.GammaTilde(self, alpha, beta)
        
            print('Expectation done')
            print('Starting maximization step')
            
            gamma_tilde_reshaped = BatchHIL_param_simplified.GammaTildeReshape(gamma_tilde, self.option_space)
            gamma_actions = BatchHIL_param_simplified.GammaReshapeActions(T, self.option_space, self.action_space, gamma, self.Labels)
            gamma_reshaped_options = BatchHIL_param_simplified.GammaReshapeOptions(gamma)
            m,n,o = gamma_actions.shape
            auxiliary_vector = np.zeros((m,n))
            for l in range(m):
                for k in range(n):
