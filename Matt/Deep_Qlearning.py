...
class Q_learning_NN:
    ...
    def NN_model(self):
        model = keras.Sequential([             
                ...
                keras.layers.Dense(256, activation='relu', 
                                   kernel_initializer=keras.initializers.RandomUniform(minval=-0.5, maxval=0.5, seed=0),
                                   bias_initializer=keras.initializers.Zeros()),                         
                keras.layers.Dense(self.env.action_size, kernel_initializer=keras.initializers.RandomUniform(minval=-0.5, maxval=0.5, seed=0))
                                 ])           
    ...
    def Training_buffer(self, NEpisodes, seed, reset = 'random', initial_state = np.array([0,0,0,8])):
        ...
        batch_size = 512
        Buffer = ReplayBuffer(32000, self.observation_space_size)
        count = 0
