...
class TD0_NN:
    def __init__(self, seed, Folder, expert_traj, option_space):
    ...
    def Option_Evaluation(self, NEpisodes, seed, pi_hi_weights, pi_lo_weights, pi_b_weights, reset = 'random', initial_state = np.array([0,0,0,8])):
...
# pool = multiprocessing.Pool(processes=option_space)
# pool = MyPool(option_space)
# args = [(option, NEpisodes, seed, pi_hi_batch, pi_lo_batch, pi_b_batch, Folders, Rand_traj, option_space) for option in range(option_space)]
# results_TD0 = pool.starmap(Hpolicy_evaluation, args) 
# pool.close()
# pool.join()

TD = TD0_NN(seed, Folders, Rand_traj, option_space)
results_TD0 = []
results_TD0 = [TD.Option_Evaluation(NEpisodes, seed, pi_hi_batch, pi_lo_batch, pi_b_batch, reset = 'standard', initial_state = np.array([0, -2.6, 0, 8]))]

# %%

with open('RL_algorithms/TD0/Results/results_TD0.npy', 'wb') as f:
    np.save(f, results_TD0)
