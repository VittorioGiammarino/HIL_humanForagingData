...
def PreprocessData(folder_name, action_space):
    ...
    for row in moving_actor:
        try:
            ...
            temp[0,:] = [row[0],row[1],row[3],row[4],row[6], 0]
    ...
    for i in range(len(data_manip)):
        ...
        if actions[index] == 360:
            index = 0
        data_manip[i,4] = np.divide((actions[index])*np.pi,180)
        data_manip[i,5] = index
                    
    actions = actions[0:-1] 
    actions_rad = actions_rad[0:-1]   

    return actions, actions_rad, data_manip


def BatchTrainingSet_velocity(data_manip):
    Expert_trajs = [np.empty((0,6)) for _ in range(int(data_manip[-1,0]))]
...
def EvaluateActionSpaceDiscretization(Expert_trajs):
    
    Expert_trajs_generated = [np.empty((0,7)) for _ in range(len(Expert_trajs))]
    
    for i in range(len(Expert_trajs_generated)):
        for j in range(len(Expert_trajs[i])):
            temp = np.empty((1,7))
            if j==0:
                temp[0,:]=Expert_trajs[i][j][:]
                Expert_trajs_generated[i] = np.append(Expert_trajs_generated[i][:][:], temp, 0)
            else:
                temp[0,:]=Expert_trajs[i][j][:]
                temp[0,2]=Expert_trajs_generated[i][j-1][2] + (Expert_trajs[i][j][1]-Expert_trajs[i][j-1][1])*Expert_trajs[i][j][6]*np.cos(Expert_trajs[i][j][4])
                temp[0,3]=Expert_trajs_generated[i][j-1][3] + (Expert_trajs[i][j][1]-Expert_trajs[i][j-1][1])*Expert_trajs[i][j][6]*np.sin(Expert_trajs[i][j][4])
                Expert_trajs_generated[i] = np.append(Expert_trajs_generated[i][:][:], temp, 0)
    
    return Expert_trajs_generated
