...
# %% Generate the data set considering the discretization of the action space
...
# %% Plot a single expert trajectory (real)
...
# %% Plot Expert's Data generated by discretization action space (generated)
...
# %% Initialization for HUMAN EXPERT
option_space = 4
# action space
termination_space = 2
size_input = TrainingSet.shape[1]
...
plt.xlabel('x')
plt.ylabel('y')
plt.savefig('Figures/Figures_HCExpert/HCExpert_{}_traj{}_ActionSpace{}_OptionsSpace{}.eps'.format(folder_name, sample_traj, action_space, option_spaceHC), format='eps')


fig = plt.figure()
ax1 = plt.subplot(311)
plot_action = plt.scatter(trajHC[sample_traj][:,0], trajHC[sample_traj][:,1], c=OptionHC[sample_traj][1:], marker='x', cmap='Set1');
...
plot_option = plt.scatter(trajHC[sample_traj][:-1,0], trajHC[sample_traj][:-1,1], c=controlHC[sample_traj], marker='x', cmap='Set1');
...
plot_termination = plt.scatter(trajHC[sample_traj][:,0], trajHC[sample_traj][:,1], c=TerminationHC[sample_traj], marker='x', cmap='copper');
cbar = fig.colorbar(plot_termination, ticks=[0, 1])
plot_water = plt.plot(water_locations[:,0], water_locations[:,1], 'bx')
cbar.ax.set_yticklabels(['Same Option', 'Terminate'])
plt.xlabel('x')
plt.ylabel('y')
plt.savefig('Figures/Figures_HCExpert/HCExpert_{}_traj{}_ActionSpace{}_OptionsSpace{}_3plots.eps'.format(folder_name, sample_traj, action_space, option_spaceHC), format='eps')
...
HCTrainingSet = dataHC
HClabels = labelsHC   
    
# %% Initialization for learning from HC Policy

termination_space = 2
size_input = HCTrainingSet.shape[1]
...
HC_ED = hil.Experiment_design(HClabels, HCTrainingSet, size_input, action_space, option_spaceHC, termination_space, N, zeta, muHC, 
                           Triple, LAMBDAS, ETA, env, max_epochHC, speedHC/1000, timeHC)
...
[trajBW, controlBW, OptionBW, 
 TerminationBW, flagBW] = sim.HierarchicalPolicySim(Trained_triple, HC_ED.zeta, HC_ED.mu, HC_ED.max_epoch, Trajs, HC_ED.option_space, HC_ED.size_input, 
                                                    Labels_dict_rad, HC_ED.speed, HC_ED.time)    
# %% reor
...
[trajBW, controlBW, OptionBW, 
 TerminationBW, flagBW] = sim.HierarchicalPolicySim(Trained_triple, ED.zeta, ED.mu, ED.max_epoch, Trajs, ED.option_space, ED.size_input, 
                                                    Labels_dict_rad, ED.speed/1000, ED.time)    
# %%
